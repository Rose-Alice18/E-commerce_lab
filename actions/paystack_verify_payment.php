...
try {
    ...
    $auth_last_four = $authorization['last4'] ?? 'XXXX';
    ...
    try {
        ...
        foreach ($cart_items as $item) {
            ...
            $new_stock = intval($item['product_stock']) - intval($item['qty']);
            ...
            $stock_stmt->bind_param("ii", $new_stock, $item['p_id']);

            if (!$stock_stmt->execute()) {
                throw new Exception("Failed to update stock for product: {$item['p_id']}");
            }

            error_log("Stock updated - Product: {$item['p_id']}, Old: {$item['product_stock']}, New: $new_stock");
            $stock_stmt->close();
        }
